 ...
 * If [`MvDecryptor`][MvDecryptor] is detected, it can be used for batch decoding of RPGM (MV/MZ) resource files. (For
   ...
   [!Note]: apparently some devs hardcode encryption key into the game, making it ignore the one in config; so I added
 ...
 * RPGM VX Ace games are worked now; they don't support Web formats (in case of images, only JPEG/PNG), so
   ...
   [!Note]: video in games wasn't tested (as I haven't found an Ace game with a video resource to test if it works). Also,
...
Main menu (keyboard can be used for quick selection)  
 ...
 * `$MVDECRYPTOR` contains path to [MvDecryptor][MvDecryptor] script (used for decryption of RPGM MV/MZ resource files).
   Defaults to system-wide location (if found, otherwise empty). Note: see below for instruction on MZ games.
...
[!Note] you can also copy script into unpacked game directory and run as `./cruncher` (or `./cruncher RenPy` etc. if you
...
### Setup using Mono (see attached file)
 ...
 2. Create a script in your $PATH (e.g. `/usr/bin/RgssDecryptor`) and mark it executable (`chmod +x`)
    ...
    ```
    [!Note]: the script is now included in release files.
...
[!Note] You can get electron package from `tur-repo` for preinstalled packages
